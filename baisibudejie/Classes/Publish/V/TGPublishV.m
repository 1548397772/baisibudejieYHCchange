...
- (void)awakeFromNib {
    ...
    CGFloat buttonStratY = (ScreenH - 2 * buttonH) * 0.5;  //算是初始Y?
    for (NSInteger i = 0 ; i < titles.count; i++) {
        ...
        CGFloat buttonEndY = buttonStratY + row * (buttonH + buttonYMargin);  //应该是 对应没歌btn的 确定最终的Y
        CGFloat buttonBeginY = buttonEndY - ScreenH; //从何处而弹出来的Y    值是负的  所以从天而降
...
- (void)cancelWithCompletionBlock:(void(^)())completionBlock{
    RootV.userInteractionEnabled = NO;
    self.userInteractionEnabled = NO;
    NSInteger beginI = 2; //因为有一个取消按钮还有一个imageView 所以数量要减2
    for (NSInteger i = beginI; i < self.subviews.count; i++) {
        ...
        if (i == self.subviews.count - 1) { //是不是设置最后一个控件 消失后 做一些事情?
            [anim setCompletionBlock:^(POPAnimation *anim, BOOL finished) {
                RootV.userInteractionEnabled =YES;
                self.userInteractionEnabled = YES;
